Param (
   ...
   [parameter()][switch]$MUTUAL_AUTH,
   [parameter()][ValidateNotNullOrEmpty()][String]$MID_USERNAME=$(if(-not $MUTUAL_AUTH) {throw "MID_USERNAME is a mandatory parameter, please provide a value."}),
   [parameter()][ValidateNotNullOrEmpty()][String]$MID_PASSWORD=$(if(-not $MUTUAL_AUTH) {throw "MID_PASSWORD is a mandatory parameter, please provide a value."}),
...
if ($MUTUAL_AUTH) {
    $msiexecCMD += "AUTH_TYPE=`"MUTUAL_AUTH`" ";
} else {
